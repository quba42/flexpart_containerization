  ...
  ELEMENTAL FUNCTION erf(x)
    ...
    interface
      ELEMENTAL FUNCTION erfdp(x) RESULT(fn_val)
        IMPLICIT NONE
        INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(14, 60)  ! `Double precision'
        REAL (dp), INTENT(IN) :: x
        REAL (dp)             :: fn_val
    ...
    erf = 0.0
    erf = real(  erfdp( real(x,kind=dp) )  ,  kind=kind(erf)  )
  END FUNCTION erf
  ...
  ELEMENTAL FUNCTION erfdp(x) RESULT(fn_val)
    ...
    ! History: 
    ! * 2015-03 Stephan.Hachinger [at] lrz.de some cleanups (avoid RETURN)
    ! * 2004    Adapted by Alan.Miller [at] vic.cmis.csiro.au
    !           from Fortran 66 routine
    ! * 1993    Naval Surface Warfare Center's Mathematics Library (1993 ver)
    ...
    REAL (dp), PARAMETER :: c = .564189583547756_dp, &
                            one = 1.0_dp, &
                            half = 0.5_dp, &
                            zero = 0.0_dp
    REAL (dp), PARAMETER ::  &
